...
export default function RatingUniversity(dta: { id: string }) {
    ...
    const { data: rates, error: rateErr, isLoading: RateLoading } = useSWR(`/vote/i/university/${dta.id}`, GlobalFetcher<IGetMyVotes[]>);
    const { data: op, error: OptErr, isLoading: OptLoading } = useSWR(`/option`, GlobalFetcher<IOptionVoting[]>);

    const options = op?.filter(s => s.owner == "university")
    ...
    const Remove = async (id: number) => {
        try {
            ...
            mutate(`/vote/i/university/${dta.id}`);
            mutate(`/option`);
        } catch {
